...
//	printf("Type = %d Token = %s\n", Cur_Type_Token, Cur_Token);
	...
	if ( Cur_Type_Token == tkUnOper )
		{
			if ( strcmp( Cur_Token, "cos" ) == 0 ) { CUR_TOKEN_PP_; value = cos ( GetE(expr_file) );  }
			if ( strcmp( Cur_Token, "sin" ) == 0 ) { CUR_TOKEN_PP_; value = sin ( GetE(expr_file) );  }
			if ( strcmp( Cur_Token, "sqrt") == 0 ) { CUR_TOKEN_PP_; value = sqrt( GetE(expr_file) );  }
			if ( strcmp( Cur_Token, "ln"  ) == 0 ) { CUR_TOKEN_PP_; value = log ( GetE(expr_file) );  }
		}
	else
		{
...
//	printf("Type = %d Token = %s\n", Cur_Type_Token, Cur_Token);
	...
	//printf( "Cur Symbol GetN = %s", Cur_Token );
