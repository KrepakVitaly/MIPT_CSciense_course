...
int     Stack_Damage ( Stack_t* Stack );
...
int main()
{
	...
	Stack_Construct ( &Stack , SIZE_OF_STACK , sizeof(int) );
	...
	Stack_Dump ( &Stack, output, WRITE_LIST_NO );
	
	
	Stack_Destruct( &Stack );  //FINITA LA COMEDIA
...
void push ( Stack_t* Stack , int input_value )
{	
	if ( Stack_Damage (Stack) ) { printf("Error of pointer\n"); return;}
...
{	
	...
	if ( Stack_Damage (Stack) ) return  printf("Error of pointer\n");
